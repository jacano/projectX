//https://github.com/CitadelCore/CSERServer/blob/master/CSERLibrary/Models/IceKey.cs

using System;
using System.Collections.Generic;

namespace CSERLibrary.Models
{
    //========= Public Domain Code ================================================
    //
    ...
    public class IceKey
    {
        ...
        public void EncryptBuffer(byte[] plainText, ref byte[] cipherText)
        {
            ...
            while (position < plainText.Length)
            {
                ...
                Array.Copy(plainText, position, plainBlock, 0, 8);

                Encrypt(plainBlock, ref cipherBlock);
                Array.Copy(cipherBlock, 0, cipherText, position, 8);
                position += 8;
            }
        }
        ...
        public void DecryptBuffer(byte[] cipherText, ref byte[] plainText)
        {
            ...
            while (position < cipherText.Length)
            {
                var cipherBlock = new byte[8];
                var plainBlock = new byte[8];
                Array.Copy(cipherText, position, cipherBlock, 0, 8);

                Decrypt(cipherBlock, ref plainBlock);
                Array.Copy(plainBlock, 0, plainText, position, 8);
                position += 8;
            }
        }
